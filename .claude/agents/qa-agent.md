...
| 22 | **Webhook hardening enforcement modes (v1.25)** — `WebhookSecurityOptions.EnforcementMode = Off` skips every gate (signature / replay / rate-limit / IP allowlist) and the endpoint is byte-for-byte identical to v1.24. `Audit` runs every gate, writes a row to `IWebhookRejectionStore`, and emits `webhook_rejected_total` + `EventId 4001-4006` — but ALWAYS returns 202 (or 200 via the legacy bearer path). `Enforce` rejects with the matching 4xx (401 sig / 409 replay / 413 too-large / 429 rate-limit / 403 IP). | `WebhookSecurityPipeline.EvaluateAsync` + `Reject()` switch | `tests/integration/.../WebhookHardeningPipelineE2ETests.cs` covers all three modes end-to-end. |
| 23 | **HMAC verifier constant-time across multi-sig + rotation (v1.25)** — Stripe-style `t=…,v1=…` headers and Slack `v0=…` headers must be hashed against EVERY presented candidate × BOTH current and previous keys without short-circuit, so timing measurements cannot reveal which candidate matched or that the rotated-out key was used. | `HmacSignatureVerifier.Verify` | `tests/unit/.../Signature/StripeVectorTests.cs` + `GitHubVectorTests.Verify_accepts_with_rotated_previous_key`. |
| 24 | **Replay nonce dedup is atomic across concurrent replicas (v1.25)** — `IWebhookReplayStore.TryRegisterAsync` MUST return `true` to exactly one caller for a given `(flowId, triggerKey, nonce)` tuple under concurrent insert. SQL Server uses `INSERT ... WHERE NOT EXISTS`; PostgreSQL uses `ON CONFLICT DO NOTHING`; in-memory uses `ConcurrentDictionary.TryAdd`. The in-memory store is single-process — multi-replica deployments MUST use Sql or Postgres. | `IWebhookReplayStore` impls + `ReplayProtectionGate` | `tests/integration/.../{Sql,PostgreSql}WebhookStoresTests.cs` cover the duplicate-rejection path; `WebhookHardeningPipelineE2ETests.Full_pipeline_rejects_replay_with_409_on_second_identical_delivery` covers the full e2e path. |
| 25 | **Webhook body buffered ONCE before signature verification (v1.25)** — `WebhookRequestBuffer.ReadAsync` must read raw body bytes capped at `MaxBodyBytes` BEFORE JSON deserialization. The same byte buffer feeds both the HMAC verifier (raw signature) and the trigger payload deserializer; reading the request stream twice would yield empty bytes for the verifier. | `DashboardServiceCollectionExtensions` webhook lambda + `WebhookRequestBuffer.ReadAsync` | `WebhookHardeningPipelineE2ETests.Full_pipeline_accepts_request_with_valid_signature_*` (positive) + `Full_pipeline_rejects_oversized_body_with_413_*` (negative). |
| 26 | **WebhookReplayJanitor purges only expired entries (v1.25)** — `WebhookReplayJanitor : BackgroundService` ticks every minute (configurable) and calls `IWebhookReplayStore.PurgeExpiredAsync(now)`. Stale entries are removed; entries with `ExpiresAt > now` survive. Disposal cancels the timer cleanly without throwing `ObjectDisposedException`. | `WebhookReplayJanitor.ExecuteAsync` | `tests/unit/.../Replay/WebhookReplayJanitorTests.cs`. |
...
| 7 | **(v1.22) Engine claim guard sits at SCHEDULE time, not EXECUTE time** — In ServiceBus topic-broadcast mode (Aspire emulator without SQL filters per dotnet/aspire#11708), a single dispatched message reaches every subscription on the topic. Each subscription's processor calls `engine.RunStepAsync` directly; there is NO claim check at the top of `RunStepAsync` because the existing claim is taken at `TryScheduleStepAsync` (line 758–766). The schedule-time claim worked for Hangfire/InMemory because schedule was 1:1 with execute, but breaks under broadcast. | FIXED 2026-05-03 — proper engine refactor: claim moved from `TryScheduleStepAsync` to top of `RunStepAsync`. New `IFlowRunRuntimeStore.ReleaseStepClaimAsync` method releases the claim on Pending re-schedule + `RetryStepAsync`. Implemented in InMemory, SQL Server, PostgreSQL. The earlier process-local dedup map in `ServiceBusFlowProcessorHostedService` is removed — engine handles broadcast natively. | Regression: `FlowOrchestratorEngineInvariantTests.RunStepAsync_WhenTryClaimStepReturnsFalse_StepExecutorIsNeverCalled` (claim losers exit silently, executor not called); `TriggerAsync_DoesNotConsultRuntimeClaim_PerV122ScheduleSemantics` (pin: schedule path no longer claims); `RunStepAsync_WhenPending_AlsoReleasesStepClaim_v122` (Pending releases the claim alongside dispatch). |
...
| 10 | **(v1.22) Cron drift on consumer backlog** — `ServiceBusCronProcessorHostedService.OnMessageAsync` recomputed the next fire as `ComputeNext(envelope.Cron, _timeProvider.GetUtcNow())` (drain time) rather than `envelope.ScheduledFor`. A backlogged consumer skipped ticks; net effect: a 5 s cron became "5 s + accumulated drain delay" forever after a single slow drain. | FIXED 2026-05-03 — `OnMessageAsync` now bases `ComputeNext` on `envelope.ScheduledFor` (the tick we just consumed); a "now or later" clamp ensures a deeply-backlogged consumer takes at most ONE catch-up tick per drain rather than bursting through hundreds of missed ticks. Net: cron stays on its declared cadence. | Regression: covered by the existing `EverySecondCron_FiresMultipleTicks_ThroughEmulator` integration test (cadence drift would manifest as fewer than 3 ticks in 30 s). |
