...
| Property | Purpose |
|---|---|
| `Id` | Stable identifier used as the database primary key and Hangfire job routing key. **Never change after deployment.** |
...
| `TriggerType.Cron` | `cronExpression` | Hangfire recurring job on the given cron schedule |
...
A `RunId` is a `Guid` generated by `TriggerAsync` for every new run. It scopes everything:
